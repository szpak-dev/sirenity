...
from ..values.junit import SirenJunitEvidence
from ..values.scenario import SirenBddScenario
...
class SirenCucumberEvidenceReader(SirenBddEvidenceReader):
    ...
    def read(self, cucumber_report: Path, feature_directory: Path) -> tuple[SirenBddFeature, ...]:
        document = TypeAdapter(list[dict[str, JsonValue]]).validate_json(cucumber_report.read_text())
        if not document:
    ...
    def feature(self, value: dict[str, JsonValue], expected_failures: frozenset[str]) -> SirenBddFeature:
        name = value["name"]
        scenarios = value["elements"]
        if not name:
            raise SirenityError("Cucumber report feature must have a name.")
        if not scenarios:
            raise SirenityError(f"Cucumber report feature {name!r} must contain scenarios.")
        return SirenBddFeature(
            name=name,
            scenarios=tuple(self.scenario(value, expected_failures) for value in scenarios),
        )

    def scenario(self, value: dict[str, JsonValue], expected_failures: frozenset[str]) -> SirenBddScenario:
        identifier = value["id"]
        name = value["name"]
        steps = value["steps"]
        if not identifier:
            raise SirenityError("Cucumber report scenario must have an identifier.")
        if not name:
            raise SirenityError("Cucumber report scenario must have a name.")
        if not steps:
            raise SirenityError(f"Cucumber report scenario {name!r} must contain steps.")
        statuses = tuple(self.status(value, name) for value in steps)
        if all(status == "passed" for status in statuses):
            if identifier in expected_failures:
                raise SirenityError(f"Cucumber report scenario {name!r} unexpectedly passed.")
            return SirenBddScenario(identifier=identifier, name=name, implemented=True)
        if (
            identifier in expected_failures
            and statuses[-1] == "skipped"
        ...
        raise SirenityError(f"Cucumber report scenario {name!r} has unexpected results: {detail}.")
    ...
    def junit(self, junit_report: Path) -> SirenJunitEvidence:
        ...
        for testcase in document.findall(".//testcase"):
            ...
            if not name:
                raise SirenityError("JUnit report contains a testcase without a name.")
            if name in identifiers:
                raise SirenityError(f"JUnit report has duplicate testcase names: {name}.")
            identifiers.add(name)
            if testcase.find("failure") is not None or testcase.find("error") is not None:
                raise SirenityError(f"JUnit report contains an unsuccessful testcase: {name}.")
            skipped = testcase.find("skipped")
            if skipped is None:
                continue
            if skipped.get("type") != "pytest.xfail":
                raise SirenityError("JUnit report contains a skipped test that is not a strict expected failure.")
    ...
    def reconcile(
        ...
        if len(reported) != len(set(reported)):
            raise SirenityError("Cucumber report contains duplicate scenarios.")
        ...
        if missing:
            ...
            raise SirenityError(f"Cucumber report is missing committed scenarios: {labels}.")
        ...
        if unexpected:
            ...
            raise SirenityError(f"Cucumber report contains unexpected scenarios: {labels}.")
        identifiers = tuple(scenario.identifier for feature in features for scenario in feature.scenarios)
        if len(identifiers) != len(set(identifiers)):
            raise SirenityError("Cucumber report contains duplicate scenario identifiers.")
        ...
        if missing_junit:
            ...
            raise SirenityError(f"JUnit report is missing Cucumber scenarios: {names}.")
        unexpected_junit = junit.identifiers.difference(cucumber_identifiers)
        if unexpected_junit:
            names = ", ".join(sorted(unexpected_junit))
            raise SirenityError(f"JUnit report contains non-Cucumber testcases: {names}.")

    def status(self, value: dict[str, JsonValue], scenario: str) -> str:
