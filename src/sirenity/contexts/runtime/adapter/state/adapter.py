...
class SirenAdapter(BaseState):
    ...
    Explicit profiles form an ordered pipeline over fresh serialized payloads and deep-copied
    ...
    def validate_routes(self) -> "SirenAdapter":
        ...
        for route in self.routes:
            for template in dict.fromkeys((route.source_path, route.public_path)):
                ...
                canonical_parts = tuple("{}" if part.startswith("{") and part.endswith("}") else part for part in parts)
    ...
    def match(self, method: str, path: str) -> SirenAdapterMatch | None:
        ...
        for route in self.routes:
            ...
            for template in dict.fromkeys((route.source_path, route.public_path)):
                ...
                if matches:
                    specificity = tuple(
                        int(not (part.startswith("{") and part.endswith("}"))) for part in template_parts
                    )
    ...
    def dispatch_path(self, method: str, path: str) -> str | None:
        ...
        if len(routes) != 1:
            raise SirenityError(f"Siren adapter operation requires exactly one route: {match.operation_id}")
        route = routes[0]
        ...
        if self.profiles and operation_id is not None:
            ...
            for route in self.routes:
                if route.operation_id not in operation_inputs:
                    ...
                    operation_inputs[route.operation_id] = value.model_copy(deep=True) if value is not None else None
            ...
            for profile in self.profiles:
                payload = dict(
                    profile.apply(
                        operation_id=operation_id,
                        operation_input=(
                            operation_input.model_copy(deep=True) if operation_input is not None else None
                        ),
                        operation_inputs={
    ...
    def capabilities(self, operation_id: str) -> frozenset[str]:
        ...
        if len(resources) != 1:
            raise SirenityError(f"Siren operation references unknown resource: {operation_id}")
    ...
    def error(self, request: SirenAdapterRequest) -> SirenDocument:
        if request.status < 400:
            raise SirenityError("An unmatched successful response cannot be projected")
